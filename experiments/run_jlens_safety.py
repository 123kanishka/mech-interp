#!/usr/bin/env python3
"""Run the scalar J-Lens safety-steering protocol."""
...
def main():
    ...
    parser.add_argument(
        "--config", type=Path, default=ROOT / "configs/jlens_safety.yaml"
    )
    parser.add_argument("--run-dir", type=Path, required=True)
    parser.add_argument(
        "--stage",
        choices=("prepare", "preflight", "validate", "test-shard", "full", "report"),
        required=True,
    )
    parser.add_argument("--shard-index", type=int)
    parser.add_argument("--shard-count", type=int)
    parser.add_argument(
        "--allow-unestimated-run",
        action="store_true",
    ...
    if root not in run.parents:
        parser.error(
            f"Use a new subdirectory of {root}; old experiment output roots are protected"
        )
    ...
    with (run / ".lock").open("a") as lock:
        ...
        if manifest.exists():
            ...
            comparable = (
                ("config", "code_sha256") if args.stage == "report" else identity.keys()
            )
            if any(original[k] != identity[k] for k in comparable):
                raise ValueError(
                    "Config/code/dependencies changed. Use a NEW run directory; never mix results."
                )
            ...
            if any(p.name != ".lock" for p in run.iterdir()):
                raise ValueError(
                    "Refusing to adopt a nonempty unmanifested run directory"
                )
        ...
        try:
            ...
            if args.stage == "preflight":
                ...
                test_shard(
                    config, run, prompts, budget, args.shard_index, args.shard_count
                )
