#!/usr/bin/env python3
"""Fresh signed-token protocol. No implicit rental, upload, or legacy-run reuse."""
...
from jlens_safety.common import Budget, BudgetExceeded, atomic_json, digest, file_hash, source_hash
...
def main():
    ...
    if root != (ROOT/'results/safety_runs_v3').resolve() or root not in run.parents:
        parser.error('Use a NEW subdirectory of results/safety_runs_v3; historical results are protected')
    if args.stage in ('test-shard','fit-shard','validate-shard') and (args.shard_index is None or not 0 <= args.shard_index < args.shard_count):
        parser.error('Sharded stages require 0 <= shard-index < shard-count')
    run.mkdir(parents=True,exist_ok=True)
    identity=dict(config=c,code_sha256=source_hash(ROOT),runner_sha256=file_hash(Path(__file__)))
    with (run/'.lock').open('a') as lock:
        ...
        if (run/'SUCCESS').exists() and args.stage!='report':
            print('Run already complete:',run);return 0
        budget=Budget(run/'budget.json',c['budget']['hard_hours'])
        try:
            if args.stage not in ('prepare','report'):
                packages={}
                for name in ('torch','transformers','numpy','scikit-learn','huggingface-hub',
                             'flash-linear-attention','fla-core','causal-conv1d','accelerate'):
                ...
                if runtime.exists() and json.loads(runtime.read_text())!=packages:
                    raise ValueError('GPU dependency versions changed across stages or workers')
            ...
            elif args.stage=='freeze': freeze_validated(c,run,rows)
            elif args.stage=='validate': fit_and_validate(c,run,rows,budget)
            elif args.stage=='full': full(c,run,rows,budget)
            elif args.stage=='test-shard': run_test_shard(c,run,rows,budget,args.shard_index,args.shard_count)
            elif args.stage=='official-judge':
                from jlens_safety.official_judge import judge_harmbench
                judge_harmbench(c,run,budget)
            elif args.stage=='report': final_report(c,run,rows)
            atomic_json(run/'state.json',dict(stage=args.stage,status='COMPLETE',scientific_run_complete=(run/'SUCCESS').exists()))
            print(args.stage,'complete:',run);return 0
            ...
            print(status+': '+str(exc),file=sys.stderr);return 2 if status=='BUDGET_STOP' else 1
        finally:
            budget.save()
