...
spec = importlib.util.spec_from_file_location(
    "vast_v3_campaign", ROOT / "scripts/vast_v3_campaign.py"
)
...
class CampaignTests(unittest.TestCase):
    def test_merge_requires_identical_provenance_and_complete_fits(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for index, p in enumerate((a, b)):
                atomic_json(
                    p / f"learned_coefficients_shard_{index}.json",
                    [dict(row, coefficients=[0.0] * 32) for row in expected[index::2]],
                )
    ...
    def test_validation_merge_deduplicates_shared_screening_and_rejects_conflicts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (b / "validation_generations.jsonl").write_text(
                json.dumps(dict(id="shared", value=999)) + "\n"
            )
