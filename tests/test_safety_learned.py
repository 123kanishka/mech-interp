...
from jlens_safety.protocol_v3 import (validate_config, scalar_conditions, score_conditions,
    freeze, fitting_conditions, verify_extra_hashes, workload, shard_conditions, validation_shard_ids)
from jlens_safety.report_v3 import factorial_comparisons, final_report
...
class LearnedCoreTests(unittest.TestCase):
    def test_two_gpu_shards_cover_fits_and_validation_exactly_once(self):
        ...
        for condition in conditions:
            ...
            self.assertEqual(assignments[0][name]|assignments[1][name],{str(i) for i in range(11)})
        with self.assertRaises(ValueError): shard_conditions(conditions,2,2)
    ...
    def test_signed_token_update_and_zero_identity(self):
        artifacts={15:dict(j_dictionary=np.eye(32),residual_scale=10.)}
        beta=[.012,-.007,.003]+[0.]*29
        c=dict(method='jlens_tokens',layer=15,seed=11,coefficients=beta,
            max_update_norm=.1,positions='last')
        ...
        zero=token_intervention({**c,'coefficients':[0.]*32},artifacts)
        torch.testing.assert_close(zero(None,None,h),h,rtol=0,atol=0)
        with self.assertRaises(ValueError):
            token_intervention({**c,'coefficients':[1.]*32},artifacts)

    def test_all_positions_and_cached_decode(self):
        h=torch.zeros(1,4,2)
        hook=Intervention([1,-1],2,.5,'all',positions='all')
        changed=hook(None,None,(h,'cache'))
        torch.testing.assert_close(changed[0],torch.tensor([[[1.,-1.]]]).expand_as(h))
    ...
    def test_total_norm_matches_last_position_prefill_energy(self):
        h=torch.zeros(1,9,2)
        last=Intervention([1,0],1,.1,'all')
        all_tokens=Intervention([1,0],1,.1,'all',positions='all',norm_matching='total')
        self.assertAlmostEqual(float(last(None,None,h).norm()),float(all_tokens(None,None,h).norm()),places=6)
        self.assertAlmostEqual(last.squared_update_norm,all_tokens.squared_update_norm,places=6)

    def test_teacher_forcing_masks_match_inference_positions(self):
        self.assertEqual(steering_mask(7,4,'last').flatten().tolist(),[0,0,0,1,1,1,1])
        self.assertEqual(steering_mask(7,4,'all').flatten().tolist(),[1]*7)
        beta=torch.nn.Parameter(torch.tensor([.02,-.03]))
        hook=differentiable_hook(beta,torch.eye(2),10,4,'last')
        output=hook(None,None,torch.zeros(1,7,2))
        output.sum().backward()
        torch.testing.assert_close(beta.grad,torch.tensor([40.,40.]))

    def test_norm_constraint_handles_collinear_cancellation(self):
        dictionary=torch.tensor([[1.,0.],[1.,0.]])
        beta=torch.nn.Parameter(torch.tensor([10.,-10.]))
        constrain_coefficients(beta,dictionary,.1)
        torch.testing.assert_close(beta,torch.tensor([10.,-10.]))
        beta.data.copy_(torch.tensor([2.,1.]))
        constrain_coefficients(beta,dictionary,.1)
        self.assertAlmostEqual(float((beta@dictionary).norm().detach()),.1,places=6)

    def test_fitting_resume_matches_uninterrupted_adam(self):
        c=config();c['learning'].update(epochs=1,accumulation_steps=1,save_every_steps=1)
        rows=[dict(id=str(i),split='train') for i in range(5)]
        condition=dict(layer=15,seed=11,method='jlens_tokens',positions='all')
        def target():
            model=torch.nn.Linear(2,2)
            return SimpleNamespace(model=model,artifacts={15:dict(j_dictionary=np.eye(2))})
        def objective(target,row,beta,dictionary,layer,positions,c):
            loss=((beta-torch.tensor([.03,-.02]))**2).sum()
            return loss,dict(nll=float(loss.detach()),truncated=False)
        class Stop:
            def __init__(self): self.calls=0
            def check(self):
                self.calls+=1
                if self.calls==3: raise RuntimeError('interrupted')
        with tempfile.TemporaryDirectory() as temp, patch('jlens_safety.learned.coefficient_loss',side_effect=objective):
            root=Path(temp)
            with self.assertRaisesRegex(RuntimeError,'interrupted'):
                fit_coefficients(target(),rows,condition,c,root/'resume.npz',Stop())
            budget=Budget(root/'budget.json',1)
            resumed=fit_coefficients(target(),rows,condition,c,root/'resume.npz',budget)
            clean_target=target()
            clean=fit_coefficients(clean_target,rows,condition,c,root/'clean.npz',budget)
            np.testing.assert_allclose(resumed['coefficients'],clean['coefficients'],rtol=0,atol=0)
            self.assertGreater(resumed['coefficients'][0],0)
            self.assertLess(resumed['coefficients'][1],0)
            self.assertTrue(all(not p.requires_grad for p in clean_target.model.parameters()))

    def test_fitting_rejects_validation_and_test(self):
        with self.assertRaisesRegex(ValueError,'training'):
            fit_coefficients(None,[dict(split='validation')],{},config(),Path('/tmp/unused'),None)
...
class LearnedDataTests(unittest.TestCase):
    def test_malformed_training_rows_are_audited_but_test_never_dropped(self):
        bad=dict(data_type='adversarial_benign',vanilla='base',adversarial='',completion='answer')
        audit=[]
        self.assertEqual(wild_rows([bad],invalid_training_rows=audit),[])
        self.assertEqual(audit,[dict(id='wj-train-0',category='adversarial_benign',reason='missing_prompt')])
    ...
    def test_remote_benchmark_data_is_never_executed(self):
        ...
        self.assertEqual(parse_published_templates("raise RuntimeError('do not execute')\nJAILBREAKS = ['one','two']"),['one','two'])
        self.assertEqual(parse_official_template("raise RuntimeError('do not execute')\nLLAMA2_CLS_PROMPT = {'prompt':'{behavior}: {generation}'}"),'{behavior}: {generation}')
    ...
    def test_group_allocation_excludes_test_and_preserves_variant_groups(self):
        ...
        for i in range(100):
            for kind in TYPES:
                ...
                records.append(dict(data_type=kind,vanilla=base,adversarial=f'{kind} variant {i}',completion='reference'))
        training=wild_rows(records)
        evaluation=wild_rows([dict(data_type='adversarial_harmful',vanilla='base 0 True',adversarial='held out')],True)
        ...
        self.assertTrue(all(r['split']=='test' for r in chosen if r['group']==test_group))

    def test_cross_corpus_base_overlap_is_linked(self):
        rows=[dict(group='a',prompt='Some request',base_prompt='Some request'),
    ...
    def test_attack_suite_required_and_exact_coverage(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            path=Path(temp)/'cases.json';path.write_text(json.dumps({'behavior':['test case']}))
            c['data'].update(harmbench_attack_file=str(path),harmbench_attack_source='published test fixture')
            rows,sha=add_harmbench_attacks([dict(id='hb-behavior',behavior_id='behavior',group='g')],c)
...
def fixture_rows():
    ...
    for split in ('train','validation','test'):
        for i,kind in enumerate(TYPES):
            result.append(dict(id=f'{split}-{i}',prompt=f'{split} example {i}',group=f'{split}-{i}',
                split=split,corpus='wildjailbreak',category=kind,harmful=int(kind.endswith('_harmful')),
                completion='reference',answer=None))
        if split!='train':
            result.append(dict(id=f'{split}-math',prompt=f'{split} math',group=f'{split}-math',split=split,
                corpus='gsm8k',category='math',harmful=0,answer='2'))
    return result


def fake_generation(prompt,condition):
    return dict(**{k:v for k,v in prompt.items() if k!='id'},id=job_id(prompt,condition),
        prompt_id=prompt['id'],condition=condition,response='#### 2',generated_ids=[2],
        truncated=False,hook_applied=0,hook_calls=1,mean_relative_intervention_norm=0.,seconds=.1,
        trace=[dict(layer=l,step=0,j_score=float(prompt['harmful']),probe_score=float(prompt['harmful'])) for l in (3,15,27,30)])
...
class LearnedPipelineTests(unittest.TestCase):
    def test_complete_v3_orchestration_and_resume_without_gpu(self):
        ...
        c=config();c['analysis'].update(screening_per_type=1,screening_gsm8k=1,diagnostic_per_stratum=1)
        prompts=fixture_rows();by_text={r['prompt']:r for r in prompts};counts={'generation':0,'fit':0}
        class FakeTarget:
            def __init__(self,c): self.artifacts={}
        ...
        class FakeJudge:
            ...
            def score(self,text,response):
                return dict(parse_error=False,response_harmful=bool(by_text[text]['harmful']),refusal=False)
            def close(self): pass
        def fake_fit(c,run,rows,budget):
            counts['fit']+=1
            atomic_npz(run/'directions.npz',**{'15__j_direction':np.ones(32)})
            atomic_json(run/'directions.json',dict(sha256=file_hash(run/'directions.npz'),
                metadata={'15':dict(token_ids=list(range(32)),tokens=[str(i) for i in range(32)])}))
        def fake_coefficients(target,rows,condition,c,checkpoint,budget):
            return dict(coefficients=[0.]*32,max_update_norm=.1)
        with tempfile.TemporaryDirectory() as temp, patch.object(protocol,'Target',FakeTarget), \
             patch.object(legacy,'Target',FakeTarget),patch.object(legacy,'Judge',FakeJudge), \
            ...
            run=Path(temp);budget=Budget(run/'budget.json',1)
            atomic_json(run/'preflight.json',dict(within_budget=True,informative=True,
                generation_limit_adequate=True,backward_and_zero_hooks_passed=True))
            protocol.full(c,run,prompts,budget)
            costs=workload(c,prompts)
            self.assertEqual(len(rows_by_id(run/'validation_generations.jsonl')),costs['validation_generations'])
            self.assertEqual(len(rows_by_id(run/'test_generations.jsonl')),costs['test_generations'])
            ...
            before=counts.copy();protocol.full(c,run,prompts,budget)
            self.assertEqual(before,counts)
            self.assertEqual(json.loads((run/'manual_review_blinded.json').read_text())[0]['notes'],review[0]['notes'])
    ...
    def test_no_arbitrary_sign_wins_when_baseline_ties(self):
        c=config();conditions=scalar_conditions(c,[15])
        from jlens_safety.analysis import attach_judgements
        rows=[];judgements={}
        for condition in conditions:
            for p in fixture_rows():
                if p['split']!='validation': continue
                row=fake_generation(p,condition);rows.append(row)
                judgements[row['id']]=dict(parse_error=False,response_harmful=False,refusal=False)
    ...
    def test_freeze_disjoint_shards_and_complete_report(self):
        c=config();c['learning']['seeds']=[11];c['analysis']['diagnostic_per_stratum']=1
        c['analysis']['screening_per_type']=1;c['analysis']['screening_gsm8k']=1
        prompts=fixture_rows();scalars=scalar_conditions(c,[15])
        learned=[dict(**condition,coefficients=[0.]*32,max_update_norm=.1)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            for condition in scalars+learned:
                for p in prompts:
                    ...
                    append_jsonl(run/'validation_judgements.jsonl',dict(id=r['id'],parse_error=False,
                        response_harmful=bool(p['harmful']),refusal=False))
            ...
            sets=[{(name,p) for name,ids in shard.items() for p in ids} for shard in shards]
            self.assertFalse(sets[0]&sets[1])
            for condition in frozen['conditions']:
                for p in prompts:
                    if p['split']!='test' or (condition['name'],p['id']) not in sets[0]|sets[1]: continue
                    r=fake_generation(p,condition)
                    append_jsonl(run/'test_generations.jsonl',r)
                    append_jsonl(run/'test_judgements.jsonl',dict(id=r['id'],parse_error=False,
                        response_harmful=bool(p['harmful']),refusal=False))
            ...
            self.assertFalse(json.loads((run/'claim_readiness.json').read_text())['publication_claims_ready'])
            atomic_json(run/'learned_coefficients.json',[])
            with self.assertRaisesRegex(ValueError,'changed'): verify_extra_hashes(run)
    ...
    def test_tiny_qwen_backward_reaches_beta_only_for_both_scopes(self):
        ...
        cfg=Qwen3_5TextConfig(vocab_size=64,hidden_size=32,intermediate_size=64,
            ...
            rope_parameters={'rope_type':'default','rope_theta':10000.,'partial_rotary_factor':.5,'mrope_section':[2,1,1]})
        ...
        for positions in ('last','all'):
            ...
            handle=model.model.layers[0].register_forward_hook(differentiable_hook(beta,torch.eye(32),1,4,positions))
            try:
                logits=model(ids,use_cache=False,logits_to_keep=3).logits
                torch.nn.functional.cross_entropy(logits.reshape(-1,64),torch.tensor([9,11,13])).backward()
