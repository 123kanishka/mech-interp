...
from jlens_safety.analysis import (
    attach_judgements,
    choose,
    cluster_interval,
    correctness,
...
from jlens_safety.pipeline import (
    BASELINE,
    check_frozen,
    freeze_test,
    job_id,
...
class SafetyCoreTests(unittest.TestCase):
    ...
    def test_test_shards_are_disjoint_complete_and_balanced(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            run = Path(temp)
            conditions = [BASELINE] + [
                dict(name=f"c{i}", method="baseline", layer=None, alpha=0.0, mode="all")
                for i in range(11)
            ]
            schedule = dict(
                primary_conditions=["baseline", "c0", "c1", "c2"],
            ...
            for filename in (
                ...
                "validation_judgements.jsonl",
            ):
                (run / filename).write_text("")
                frozen[filename.replace(".jsonl", "") + "_sha256"] = __import__(
                    "jlens_safety.common", fromlist=["file_hash"]
                ).file_hash(run / filename)
            atomic_npz(run / "directions.npz", x=np.ones(1))
            frozen["directions_sha256"] = __import__(
                "jlens_safety.common", fromlist=["file_hash"]
            ).file_hash(run / "directions.npz")
    ...
    def test_direction_projection(self):
        ...
        dictionary = np.array([[1.0, 0.0, 0.0], [2.0, 0.0, 0.0]])
    ...
    def test_fit_gate_uses_only_training_benign(self):
        ...
        x[y == 1, 0] += 3
        a = fit_layer(x, y, np.eye(4), np.eye(4), 1, 0.9)
        scores = (x[y == 0] @ a["j_direction"] - a["gate_mean"]) / a["gate_scale"]
        self.assertAlmostEqual(a["gate_threshold"], np.quantile(scores, 0.9), places=5)
        self.assertGreater(
            np.dot(a["j_direction"], x[y == 1].mean(0) - x[y == 0].mean(0)), 0
        )
    ...
    def test_hook_zero_exact_tuple_and_prompt_positions(self):
        ...
        hook = Intervention([1, 0, 0, 0], 2, 0.5, "all")
        ...
        torch.testing.assert_close(
            changed[0][:, -1] - h[:, -1], torch.tensor([[1.0, 0, 0, 0]])
        )
    ...
    def test_gate_is_frozen_for_decode(self):
        hook = Intervention([1, 0], 1, 1, "gated", gate=([1.0, 0.0], 0, 1, 1))
        h = torch.zeros(1, 4, 2)
    ...
    def test_random_control_reproducible_and_unit(self):
        ...
        condition = dict(
            method="random", layer=3, random_seed=11, alpha=0.1, mode="all"
        )
    ...
    def test_group_split_and_leakage(self):
        rows = [
            dict(
                id=f"{g}-{y}", prompt=f"example {g} class {y}", group=str(g), harmful=y
            )
            for g in range(60)
            for y in (0, 1)
        ]
    ...
    def test_asymmetric_group_counts_and_frozen_subsets(self):
        ...
        counts = dict(
            train=dict(safe=4, unsafe=2),
            validation=dict(safe=4, unsafe=2),
        ...
        for split, wanted in counts.items():
            for label, value in wanted.items():
                self.assertEqual(
                    sum(
                        r["split"] == split and r["harmful"] == int(label == "unsafe")
                        for r in result
                    ),
                    value,
                )
...
def fake_generation(prompt, condition):
    return dict(
        **{k: v for k, v in prompt.items() if k != "id"},
        ...
        hook_applied=0,
        hook_calls=1,
        mean_relative_intervention_norm=0,
        seconds=0.1,
        trace=[
            dict(
                layer=layer,
...
class SafetyPipelineTests(unittest.TestCase):
    def test_full_orchestration_resume_without_gpu(self):
        ...
        class FakeTarget:
            ...
            def extract(self, prompt):
                return {
                    layer: np.array([1.0, 2.0, 3.0, 4.0]) for layer in (3, 15, 27, 30)
                }

            def fit(self, cache, labels):
                artifact = dict(
                    j_direction=np.array([1.0, 0, 0, 0]),
                    residual_direction=np.array([1.0, 0, 0, 0]),
                    logit_direction=np.array([1.0, 0, 0, 0]),
                    gate_mean=0.0,
                    gate_scale=1.0,
                    gate_threshold=1.0,
                    residual_scale=1.0,
                    probe_weight=np.array([1.0, 0, 0, 0]),
                    probe_bias=0.0,
                )
                return {layer: artifact for layer in (3, 15, 27, 30)}, {}
        ...
        prompts = [
            dict(
                id=f"{split}-{i}",
                prompt=f"placeholder {split}-{i}",
                split=split,
                harmful=int(i % 3 == 0),
                corpus="gsm8k" if i % 3 == 2 else "xstest",
                group=f"{split}-{i}",
                category="test",
                answer="2" if i % 3 == 2 else None,
            )
        ...
        with (
            ...
            patch("jlens_safety.pipeline.Target", FakeTarget),
            patch("jlens_safety.pipeline.Judge", FakeJudge),
        ):
            run = Path(temp)
            ...
            self.assertEqual(
                counts["generate"], 37 * 3 + 10 * 3 + 6 * 6 + 4 * 6 + 8 * 3
            )
            ...
            self.assertEqual(
                (diagnostic["n_prompts"], diagnostic["n_generations"]), (3, 36)
            )
    ...
    def test_tiny_qwen_hybrid_cached_generation(self):
        ...
        with torch.inference_mode():
            ...
            nonzero = Intervention(np.ones(32) / np.sqrt(32), 1, 0.1, "prefill")
    ...
    def test_selection_freeze_report_and_resume_integrity(self):
        ...
        prompts = [
            dict(
                ...
                split="validation",
                harmful=int(i == 0),
                corpus="gsm8k" if i == 2 else "xstest",
                answer="2" if i == 2 else None,
                category="test",
                group=f"g{i}",
            )
            for i in range(3)
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue(
                all(e["paired_delta"]["mean"] == 0 for e in summary["estimates"])
            )
            with self.assertRaises(ValueError):
