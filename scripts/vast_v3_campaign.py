#!/usr/bin/env python3
"""Run a detached two-host campaign with optional saved-pilot reuse."""
...
def checked_merge(destination, source, stage):
    ...
    if stage == "fit-shard":
        ...
        for index, folder in enumerate((destination, source)):
            items = json.loads(
                (folder / f"learned_coefficients_shard_{index}.json").read_text()
            )
            wanted = expected[index::2]
            if len(items) != len(wanted) or any(
                any(r.get(k) != v for k, v in e.items()) for r, e in zip(items, wanted)
            ):
                raise ValueError("Coefficient shard condition coverage differs")
            results.extend(items)
        by_name = {r["name"]: r for r in results}
        atomic_json(
            destination / "learned_coefficients.json",
            [by_name[r["name"]] for r in expected],
        )
    elif stage == "validate-shard":
        if file_hash(destination / "learned_coefficients.json") != file_hash(
            source / "learned_coefficients.json"
        ):
...
def wait_setup():
    ...
    while not (ROOT / "SETUP_COMPLETE").exists():
        status = subprocess.run(
            ["supervisorctl", "status", "jlens_v3_setup"],
            capture_output=True,
            text=True,
        ).stdout
...
def prefetch(config):
    ...
    for key in ("model", "judge", "official_judge"):
        ...
        if c.get("enabled", True):
            ...
            weights = (
                "*.safetensors"
                if any(p.endswith(".safetensors") for p in files)
...
def main():
    ...
    parser.add_argument(
        "--role", choices=["coordinator", "worker", "prefetch"], required=True
    )
    ...
    def execute(stage, index=None):
        command = [
            sys.executable,
            str(ROOT / "experiments/run_jlens_safety_learned.py"),
            "--config",
            str(args.config),
            "--run-dir",
            str(run),
        ...
        if stage == "inherit-pilot":
            command += ["--pilot-source", str(args.pilot_source)]
        if index is not None:
            command += ["--shard-index", str(index)]
        subprocess.run(
            command, cwd=ROOT, check=True, timeout=max(1, deadline - time.time())
        )
    ...
    if args.role == "worker":
        ...
        while time.time() < deadline:
            ...
            stage = task["stage"]
            atomic_json(
                control / "worker_status.json",
                dict(id=previous, stage=stage, status="RUNNING"),
            )
            try:
                if stage == "done":
                    atomic_json(
                        control / "worker_status.json",
                        dict(id=previous, stage=stage, status="COMPLETE"),
                    )
                    return
                if stage == "prefetch":
                    subprocess.run(
                    ...
                    raise ValueError("Unrecognized worker stage")
                atomic_json(
                    control / "worker_status.json",
                    dict(id=previous, stage=stage, status="COMPLETE"),
                )
                ...
                atomic_json(
                    control / "worker_status.json",
                    dict(id=previous, stage=stage, status="FAILED", error=str(exc)),
                )
                raise
    ...
    ssh = [
        ...
        "-o",
        "UserKnownHostsFile=" + str(args.worker_key) + ".known_hosts",
        "-i",
        str(args.worker_key),
        "-p",
    ...
    def enqueue(stage):
        ...
        remote(
            f'mv {shlex.quote(str(control/"task.next.json"))} {shlex.quote(str(control/"task.json"))}'
        )
        return task["id"]
    ...
    def wait_worker(task_id):
        while time.time() < deadline:
            text = remote(
                "test ! -f "
                + shlex.quote(str(control / "worker_status.json"))
                + " || cat "
                + shlex.quote(str(control / "worker_status.json"))
            )
            ...
            if status.get("id") == task_id:
                ...
                if status["status"] == "FAILED":
                    raise RuntimeError("Worker failed: " + str(status))
            time.sleep(60)
    ...
    def snapshot():
        ...
        sync(
            ...
            f"{endpoint}:{run}/",
            "--exclude=.lock",
            "--exclude=coefficient_checkpoints",
            "--exclude=activations",
        )
    ...
    def status(stage, state="RUNNING", **extra):
        ...
        atomic_json(
            control / "campaign_status.json",
            dict(stage=stage, status=state, deadline=deadline, **extra),
        )
    ...
    try:
        ...
        for active in ("fit-shard", "validate-shard", "test-shard"):
            ...
            if active == "test-shard":
                subprocess.run(
                    [
                        sys.executable,
                        str(ROOT / "scripts/merge_safety_shards.py"),
                        "--coordinator",
                        str(run),
                        "--worker",
                        str(mirror),
        ...
        snapshot()
        status(
            active, "COMPLETE", results_mirrored=True, external_backup_verified=False
        )
    except Exception as exc:
