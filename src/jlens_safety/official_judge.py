...
def parse_official_template(source):
    ...
    for node in tree.body:
        if isinstance(node, ast.Assign) and any(
            isinstance(t, ast.Name) and t.id == "LLAMA2_CLS_PROMPT"
...
def template_for_run(c, run):
    ...
    if path.exists():
        ...
        if value["source"] != url or value["template_hash"] != digest(
            value["template"]
        ):
...
class OfficialJudge:
    def __init__(self, c, run):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            max_memory={
                0: f'{spec["gpu_memory_gib"]}GiB',
                "cpu": f'{spec["cpu_memory_gib"]}GiB',
            },
        ).eval()
    ...
    def score(self, behavior, response):
        ...
        if inputs.input_ids.shape[-1] > 4096:
            raise ValueError(
                "Official classifier input exceeds 4096; do not silently truncate"
            )
        ...
        for r in wanted:
            ...
            append_unique(
                ...
                dict(
                    ...
                    **judge.score(r["behavior"], r["response"]),
                ),
                known,
            )
    finally:
