...
def validation_power(rows, c):
    ...
    for name in sorted(
        {
            r["condition"]["name"]
            for r in rows
            if r["condition"]["method"] == "jlens_tokens"
        }
    ):
        ...
        for r in rows:
            if r["condition"]["name"] == name and r["harmful"]:
                groups[r["group"]].append(
                    ...
                    - base[r["prompt_id"]]["metrics"]["harmful_compliance"]
                )
        ...
        result.append(
            dict(
                ...
                **paired_power_plan(
                    values,
                    c["analysis"]["target_absolute_effect"],
                    c["analysis"]["power"],
    ...
    return dict(
        estimates=result,
        note="Validation-only approximate paired group planning. No test-based resizing. "
...
def bootstrap_difference(values, groups, c, comparisons=5):
    ...
    return dict(
        ...
        high=float(np.quantile(estimates, 1 - tail)),
        n=len(values),
        groups=len(unique),
...
def factorial_comparisons(rows, c):
    ...
    for r in rows:
        ...
        if name in ("scalar_last", "scalar_all"):
            ...
            method == "jlens_tokens"
            and r["condition"].get("norm_matching", "per_position") == "per_position"
        ):
        ...
        for metric in ("harmful_compliance", "overrefusal", "math_accuracy"):
            for name, coefficients in weights.items():
                ...
                for pid, arms in by_prompt.items():
                    ...
                    values.append(
                        sum(
                            w * np.mean([m[metric] for m in arms[a]])
...
def final_report(c, run, prompts):
    ...
    expected_jobs = {
        job_id(p, condition): (p, condition)
        for p in prompts
        if p["split"] == "test"
    ...
    if set(generated) != expected or set(judged) != expected:
        raise ValueError(
            "Final results are missing, duplicated, or contain unexpected jobs"
        )
    for key, row in generated.items():
        prompt, condition = expected_jobs[key]
        if (
            row["condition"] != condition
            or row["prompt_id"] != prompt["id"]
            or any(
                row.get(field) != value
    ...
    if official_enabled and set(official) != official_expected:
        raise ValueError(
            "Missing or unexpected official HarmBench labels; run official-judge before report"
        )
    if official_enabled and official_expected:
        template = json.loads((run / "harmbench_classifier_template.json").read_text())[
            "template"
        ]
    ...
    primary = [
        r
        for r in rows
        if r["condition"]["name"] in frozen["schedule"]["primary_conditions"]
    ]
    summary = summarise(primary, c)
    summary["scope"] = (
        "Fresh test generations in a bounded exploratory v3 run; baseline + scalar scopes + all registered J-token fitting seeds"
    )
    if (run / "preflight.json").exists():
        summary["development_pilot_audit"] = json.loads(
            (run / "preflight.json").read_text()
        )
    summary["harmbench_metric"] = (
        "WildGuard harmful-response rate; official behavior ASR is separate"
    )
            ...
            if t["step"] == 0 and t["layer"] in traces:
                persistence.append(
                    dict(
                        ...
                        corpus=r["corpus"],
                        layer=t["layer"],
                        j_score_delta=t["j_score"] - traces[t["layer"]]["j_score"],
                        probe_score_delta=t["probe_score"]
                        - traces[t["layer"]]["probe_score"],
                        baseline_harmful_response=b["judgement"]["response_harmful"],
                        harmful_response=r["judgement"]["response_harmful"],
                    )
                )
    atomic_json(
        run / "downstream_persistence.json",
        dict(
            rows=persistence,
    ...
    atomic_json(
        run / "diagnostic_summary.json",
        summarise([r for r in rows if r["prompt_id"] in diag_ids], c),
    )
    perturbations = []
    for name in sorted({r["condition"]["name"] for r in rows}):
        ...
        perturbations.append(
            dict(
                condition=name,
                n=len(selected),
                mean_edited_positions=float(
                    np.mean([r.get("edited_positions", 0) for r in selected])
                ),
                mean_squared_trajectory_update=float(
                    np.mean([r.get("squared_update_norm", 0.0) for r in selected])
                ),
                mean_relative_per_call_norm=float(
                    np.mean([r["mean_relative_intervention_norm"] for r in selected])
                ),
            )
        )
                ...
                dict(
                    ...
                    terms=[
                        dict(
                            ...
                            token_id=(
                                None
                                if fit["method"] == "random_tokens"
                                else labels["token_ids"][i]
                            ),
                            token=(
                                None
                                if fit["method"] == "random_tokens"
                                else labels["tokens"][i]
                            ),
                            beta=beta,
                        )
    ...
    official_summary = []
    for name, corpus in sorted(
        {(r["condition"]["name"], r["corpus"]) for r in rows if r["id"] in official}
    ):
        selected = [
            r for r in rows if r["condition"]["name"] == name and r["corpus"] == corpus
        ]
        ...
        for r, value in zip(selected, values):
            by_behavior[r["behavior_id"]].append(value)
        official_summary.append(
            dict(
                condition=name,
                corpus=corpus,
                prompt_success=cluster_interval(
                    values,
                    [r["group"] for r in selected],
                    c["experiment"]["seed"],
                    c["analysis"]["bootstrap_samples"],
                ),
                behavior_any_success=float(
                    np.mean([max(v) for v in by_behavior.values()])
                ),
                behaviors=len(by_behavior),
                scope="Standard-text subset and registered attack suite only",
            )
        )
    ...
    if review_path.exists():
        ...
        if set(existing) != {r["review_id"] for r in review} or any(
            existing[r["review_id"]][field] != r[field]
            for r in review
    ...
    for corpus in sorted({r["corpus"] for r in primary}):
        ...
        quality.append(
            dict(
                ...
                harmful_prompts=len(b),
                baseline_failure_floor=bool(
                    b and failures < c["analysis"]["min_pilot_harmful_responses"]
                ),
                truncation=float(np.mean([r["truncated"] for r in subset])),
            )
        )
    atomic_json(
        run / "claim_readiness.json",
        dict(
            computationally_complete=True,
            publication_claims_ready=False,
            quality_by_corpus=quality,
            pending=[
                "Blinded human review and judge-agreement audit",
                "Interpret group-level uncertainty and power limits",
                "Development pilot failed informativeness and truncation gates; no replacement preflight was run",
            ]
            + (
                []
                if official_enabled
                else ["Official HarmBench classifier evaluation disabled"]
            ),
    ...
    atomic_json(
        run / "checksums.json",
        {
            ...
            if p.is_file()
            and p.name
            not in ("checksums.json", "budget.json", "state.json", ".lock", "SUCCESS")
        },
    )
    (run / "SUCCESS").touch()
...
def plots(factorial, persistence, run):
    ...
    for metric in ("harmful_compliance", "overrefusal", "math_accuracy"):
        ...
        fig, ax = plt.subplots(figsize=(10, max(3, len(values) * 0.3)))
        for i, r in enumerate(values):
            ax.errorbar(
                r["mean"],
                i,
                xerr=[[max(0, r["mean"] - r["low"])], [max(0, r["high"] - r["mean"])]],
                fmt="o",
            )
        ax.set_yticks(
            range(len(values)),
            [r["corpus"] + " / " + r["comparison"] for r in values],
            fontsize=7,
        )
        ax.axvline(0, color="grey", linestyle="--")
        ax.set_xlabel(
            metric + " paired difference; corrected intervals within each corpus/metric"
        )
        fig.tight_layout()
    ...
    for r in persistence:
        if r["condition"].startswith("jlens_tokens") and not r["condition"].endswith(
            "totalnorm"
        ):
    ...
    for name, values in sorted(buckets.items()):
        ...
        ax.plot(
            layers,
            [np.mean(values[layer]) for layer in layers],
            label=name,
            marker="o",
        )
    ax.axhline(0, color="grey")
    ax.set(xlabel="Layer", ylabel="Same-prefix prefill J-score change")
    if buckets:
        ax.legend(fontsize=6)
    fig.tight_layout()
    fig.savefig(output / "downstream.png", dpi=160)
    plt.close(fig)
