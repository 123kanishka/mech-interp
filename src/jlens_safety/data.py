...
def fetch_csv(url):
    with urllib.request.urlopen(url, timeout=60) as response:
        raw = response.read()
    return (
        list(csv.DictReader(io.StringIO(raw.decode("utf-8-sig")))),
...
def grouped_splits(rows, counts, seed):
    ...
    wanted = {
        s: (
            [counts[s], counts[s]]
            if isinstance(counts[s], int)
            else [counts[s]["safe"], counts[s]["unsafe"]]
        )
        for s in partitions[:3]
    }
    wanted["unused"] = [
        total[y] - sum(wanted[s][y] for s in partitions[:3]) for y in (0, 1)
    ]
    ...
    for r in rows:
        ...
        by_category[
            cindex[(r["harmful"], r.get("category", "unknown"))], gindex[r["group"]]
        ] += 1
    nbin = len(groups) * 4
    ...
    for p, split in enumerate(partitions):
        ...
        for y in (0, 1):
            ...
            lower[k] = upper[k] = wanted[split][y]
            k += 1
    ...
    result = milp(
        objective,
        integrality=np.r_[np.ones(nbin), np.zeros(ndev)],
    ...
    if result.x is None:
        raise ValueError(
            "No feasible focus-disjoint allocation; revise counts, not grouping"
        )
...
def scheduled_subsets(rows, config):
    result = {}
    for kind, split, key in [
        ("screening", "validation", "screening_counts"),
        ...
        for i, (role, count) in enumerate(config["data"][key].items()):
            candidates = [
                ...
                and (
                    ...
                    or (role in ("harmbench", "gsm8k") and r["corpus"] == role)
                )
            ]
            selected.extend(
                stratified_subset(candidates, count, config["experiment"]["seed"] + i)
            )
...
def prepare(config, run):
    ...
    rows.extend(
        ...
        for r in eligible[: d["harmbench_test"]]
    )
    parquet_hashes = {}
    for split, original in [("validation", "train"), ("test", "test")]:
        filename = f"main/{original}-00000-of-00001.parquet"
        path = hf_hub_download(
            "openai/gsm8k", filename, repo_type="dataset", revision=d["gsm8k_revision"]
        )
    ...
    atomic_json(
        ...
        dict(
            urls=urls,
            sha256={"xstest": xs_hash, "harmbench": hb_hash},
            gsm8k_revision=d["gsm8k_revision"],
            gsm8k_sha256=parquet_hashes,
