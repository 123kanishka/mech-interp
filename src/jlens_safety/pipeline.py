...
from .analysis import (attach_judgements, choose, diagnostic_report, save_plots,
    summarise, save_validation_plots, challenge_report)
from .backend import Judge, Target
from .data import scheduled_subsets
from .common import (append_unique, atomic_json, atomic_npz, digest, file_hash, rows_by_id)
    ...
    for method in ('jlens', 'residual', 'logit'):
        for layer in config['jlens']['intervention_layers']:
            for alpha in config['steering']['alphas']:
                result.append(dict(name=f'{method}_l{layer}_a{alpha}', method=method,
                    layer=layer, alpha=alpha, mode='all'))
    return result
...
def validate_config(config):
    ...
    if config['budget']['hard_hours'] > 14 or config['budget']['target_hours'] > config['budget']['hard_hours']:
    ...
    for split in ('train', 'validation', 'test'):
        if any(config['data']['xstest_counts'][split][label] < 1 for label in ('safe', 'unsafe')):
...
def workload(config, prompts):
    ...
    validation_conditions = len(sweep_conditions(config)) + 2 * len(config['jlens']['intervention_layers'])
    ...
    nvalidation = len(sweep_conditions(config))*nscreen + shortlist*(nval-nscreen) + 2*len(config['jlens']['intervention_layers'])*nval
    ntest_jobs = 4*ntest + (test_conditions-4)*ndiagnostic
    return dict(training_prompts=ntrain, validation_prompts=nval, test_prompts=ntest,
        validation_conditions=validation_conditions, test_conditions=test_conditions,
        screening_prompts=nscreen, diagnostic_prompts=ndiagnostic,
        primary_test_conditions=4, validation_generations=nvalidation, test_generations=ntest_jobs,
        generations=nvalidation+ntest_jobs,
        max_target_generated_tokens=(nvalidation+ntest_jobs)*config['model']['max_new_tokens'])
...
def preflight(config, run, prompts, budget, distributed_shards=1):
    ...
    if shutil.disk_usage(run).free < config['budget']['min_free_gib'] * 1024**3:
        raise RuntimeError('Insufficient free disk for target + sequential judge caches')
    ...
    get_hf_file_metadata(hf_hub_url(c['name'], 'model-00001-of-00002.safetensors', revision=c['revision']))
    selected = []
    for corpus, harmful in [('xstest', 0), ('xstest', 1), ('gsm8k', 0)]:
        subset = [p for p in prompts if p['split'] == 'validation' and p['corpus'] == corpus and p['harmful'] == harmful]
    ...
    try:
        ...
        tiny = target.generate(selected[0]['prompt'], {**zero, 'alpha': .01}, max_new_tokens=8)
    ...
    try:
        for row, generation in results:
            ...
            if result['parse_error']:
                raise RuntimeError(f'WildGuard preflight parsing failed: {result["raw"]!r}')
    ...
    seconds_per_token = max(g['seconds']/max(1, g['generated_tokens']) for _, g in results)
    generation_seconds = seconds_per_token * config['model']['max_new_tokens']
    judge_seconds = max(j['seconds'] for j in judge_results)
    coordinator_jobs = costs['validation_generations'] + int(np.ceil(costs['test_generations']/distributed_shards))
    estimate = config['budget']['safety_factor'] * (
        coordinator_jobs * (generation_seconds + judge_seconds)
        + costs['training_prompts'] * max(extraction_times)) + 1800
    report = dict(workload=costs, projected_remaining_hours=estimate/3600,
    ...
    atomic_json(run / 'preflight_examples.json', [dict(prompt=p, generation=g, judge=j)
        for (p, g), j in zip(results, judge_results)])
    if not report['within_budget']:
        raise RuntimeError('Projected distributed coordinator run exceeds target budget. Revise protocol before fitting/testing.')
    return report
...
def fit(config, run, prompts, budget):
    ...
    try:
        ...
        for p in train:
            ...
            if not path.exists():
                atomic_npz(path, **{str(k): v for k, v in target.extract(p['prompt']).items()})
            with np.load(path, allow_pickle=False) as content:
                cached.append({int(k): content[k] for k in content.files})
        x = {l: np.stack([r[l] for r in cached]) for l in config['jlens']['observation_layers']}
        artifacts, metadata = target.fit(x, [r['harmful'] for r in train])
        atomic_npz(run / 'directions.npz', **{f'{l}__{k}': np.asarray(v) for l, a in artifacts.items() for k, v in a.items()})
        atomic_json(run / 'directions.json', dict(metadata=metadata,
            training_ids=[r['id'] for r in train], sha256=file_hash(run / 'directions.npz')))
    finally:
        target.close()
...
def generate_jobs(config, run, prompts, conditions, split, budget, allowed_ids=None):
    ...
    jobs = [(p, c) for c in conditions for p in prompts if p['split'] == split and
            (allowed_ids is None or p['id'] in allowed_ids[c['name']])]
    ...
    try:
        for prompt, condition in jobs:
            ...
            row = {**prompt, **output, 'id': key, 'prompt_id': prompt['id'], 'condition': condition}
            append_unique(path, row, known)
            atomic_json(run / 'progress.json', dict(stage=f'{split}_generation',
                complete=len(known), requested=len(jobs), active_seconds=budget.seconds))
    finally:
        target.close()
    ...
    try:
        for row in generations:
            if row['id'] in known:
                continue
            budget.check()
            key = digest({'prompt': row['prompt'], 'response': row['response'], 'judge': config['judge']})
            if key in cache:
                result = {k: v for k, v in cache[key].items() if k not in ('id', 'input_hash')}
            else:
                ...
                if result['parse_error']:
                    ...
                    raise RuntimeError('Judge returned malformed labels; stopped without silently treating them as safe')
...
def select_initial(config, run, selection_ids=None, candidate_conditions=None,
                   filename='validation_selection.json'):
    rows = attach_judgements(read_jsonl(run / 'validation_generations.jsonl'),
    ...
    for method in ('jlens', 'residual', 'logit'):
        for layer in config['jlens']['intervention_layers']:
            candidates = [c for c in (candidate_conditions or sweep_conditions(config))
...
def freeze_test(config, run, prompts):
    ...
    layers = config['jlens']['intervention_layers']
    j_candidates = [choices[f'jlens_{l}']['condition'] for l in layers]
    best, _ = choose(rows, j_candidates, config)
    layer = best['condition']['layer']
    conditions = [BASELINE]
    for position, l in zip(('early', 'middle', 'late'), layers):
        conditions.append({**choices[f'jlens_{l}']['condition'], 'name': 'jlens_' + position})
    for mode in ('gated', 'prefill'):
        selected, _ = choose(rows, [c for c in initial['followup'] if c['mode'] == mode], config)
        conditions.append({**selected['condition'], 'name': 'jlens_' + mode})
    for method in ('residual', 'logit'):
        selected, _ = choose(rows, [choices[f'{method}_{l}']['condition'] for l in layers], config)
        conditions.append({**selected['condition'], 'name': method})
    for seed in config['steering']['random_seeds']:
        conditions.append({**best['condition'], 'name': f'random_{seed}', 'method': 'random', 'random_seed': seed})
    residual = choices[f'residual_{layer}']['condition']
    conditions.append(dict(name='combined', method='combined', layer=layer,
        j_alpha=best['condition']['alpha'], residual_alpha=residual['alpha'],
        alpha=abs(best['condition']['alpha']), mode='all'))
    best_name = 'jlens_' + dict(zip(layers, ('early', 'middle', 'late')))[layer]
    schedule = dict(primary_conditions=['baseline', best_name, 'residual', 'logit'],
        diagnostic_prompt_ids=scheduled_subsets(prompts, config)['diagnostic'])
    result = dict(conditions=conditions, conditions_hash=digest(conditions),
...
def check_frozen(run):
    ...
    for file, key in [('validation_generations.jsonl', 'validation_generations_sha256'),
                      ('validation_judgements.jsonl', 'validation_judgements_sha256'),
                      ('directions.npz', 'directions_sha256')]:
        if file_hash(run / file) != result[key]:
            raise ValueError('Frozen selection inputs changed; refuse contaminated test resume')
...
def test_allowed_ids(run, prompts):
    ...
    return {c['name']: all_ids if c['name'] in schedule['primary_conditions'] else diagnostic
...
def sharded_test_allowed_ids(run, prompts, shard_index, shard_count):
    ...
    jobs = sorted((job_id(p, c), c['name'], p['id']) for c in conditions for p in test
...
def final_report(config, run, prompts):
    ...
    expected = {job_id(p, c) for p in prompts if p['split'] == 'test' for c in conditions
                if p['id'] in allowed[c['name']]}
    if set(generated) != expected:
        raise ValueError('Missing or unexpected test jobs; cannot mark SUCCESS')
    rows = attach_judgements(list(generated.values()), rows_by_id(run / 'test_judgements.jsonl'))
    schedule = json.loads((run / 'frozen_test.json').read_text())['schedule']
    primary_rows = [r for r in rows if r['condition']['name'] in schedule['primary_conditions']]
    diagnostic_rows = [r for r in rows if r['prompt_id'] in schedule['diagnostic_prompt_ids']]
    ...
    small_summary['scope'] = 'Exploratory: all conditions on the SAME predefined diagnostic subset'
    atomic_json(exploratory / 'summary.json', small_summary)
    save_plots(small_summary, diagnostic_rows, diagnostic_report(diagnostic_rows, config), exploratory)
    validation = attach_judgements(read_jsonl(run / 'validation_generations.jsonl'),
        rows_by_id(run / 'validation_judgements.jsonl'))
    screening_ids = set(scheduled_subsets(prompts, config)['screening'])
    save_validation_plots([r for r in validation if r['prompt_id'] in screening_ids], config, run)
    ...
    for condition in conditions:
        for corpus in sorted({r['corpus'] for r in rows}):
            subset = [r for r in rows if r['condition']['name'] == condition['name'] and r['corpus'] == corpus]
            review.extend(subset[:2])
    atomic_json(run / 'manual_review_sample.json', review)
    atomic_json(run / 'checksums.json', {str(p.relative_to(run)): file_hash(p)
        for p in run.rglob('*') if p.is_file() and p.name not in ('checksums.json', 'budget.json', 'state.json', '.lock')})
...
def full(config, run, prompts, budget):
    ...
    if not (run / 'frozen_test.json').exists():
        ...
        generate_jobs(config, run, screening, sweep_conditions(config), 'validation', budget)
        judge_jobs(config, run, 'validation', budget)
        screened = select_initial(config, run, selection_ids=set(subsets['screening']),
    ...
    conditions = check_frozen(run)
    generate_jobs(config, run, prompts, conditions, 'test', budget, allowed_ids=test_allowed_ids(run, prompts))
...
def test_shard(config, run, prompts, budget, shard_index, shard_count):
    ...
    atomic_json(run / f'shard_{shard_index}_of_{shard_count}.json',
        dict(shard_index=shard_index, shard_count=shard_count, jobs=expected,
