...
def token_intervention(condition, artifacts):
    dictionary = normalized_dictionary(artifacts, condition['layer'], condition['method'], condition['seed'])
    ...
    return Intervention(update, artifacts[condition['layer']]['residual_scale'], 1., 'all',
        positions=condition['positions'], norm_matching=condition.get('norm_matching', 'per_position'))
...
def training_tensors(target, row, max_completion_tokens):
    ...
    completion = target.tokenizer(row['completion'], add_special_tokens=False)['input_ids']
...
def coefficient_loss(target, row, beta, dictionary, layer, positions, config):
    c = config['learning']
    ids, labels, prompt_length, truncated = training_tensors(target, row, c['max_completion_tokens'])
    n = labels.shape[1]
    baseline = None
    if not row['harmful'] and c['benign_kl_weight']:
        with torch.no_grad():
            baseline = target.model(input_ids=ids, attention_mask=torch.ones_like(ids),
                use_cache=False, logits_to_keep=n).logits.float().log_softmax(-1)
    hook = target.layers[layer].register_forward_hook(differentiable_hook(
        beta, dictionary, target.artifacts[layer]['residual_scale'], prompt_length, positions))
    try:
        logits = target.model(input_ids=ids, attention_mask=torch.ones_like(ids),
            use_cache=False, logits_to_keep=n).logits.float()
        if logits.shape[1] != n:
            raise ValueError('Model did not honor completion-only logits; check adapter')
        nll = F.cross_entropy(logits.reshape(-1, logits.shape[-1]), labels.reshape(-1))
        kl = logits.new_zeros(())
        if baseline is not None:
            kl = F.kl_div(logits.log_softmax(-1), baseline, log_target=True,
                          reduction='sum') / n
        loss = nll + c['benign_kl_weight'] * kl + c['coefficient_l2'] * beta.square().sum()
        if not torch.isfinite(loss):
            raise ValueError('Nonfinite coefficient loss')
        return loss, dict(nll=float(nll.detach()), kl=float(kl.detach()), truncated=truncated)
    finally:
        hook.remove()


def fit_coefficients(target, rows, condition, config, checkpoint, budget, max_examples=None):
    ...
    dictionary = torch.as_tensor(normalized_dictionary(target.artifacts, layer, condition['method'], seed), device=device)
    ...
    identity = digest(dict(condition=condition, ids=[r['id'] for r in rows], learning=c,
                           max_examples=max_examples))
    cursor, updates, truncated, seconds, nll_sum = 0, 0, 0, 0., 0.
    if checkpoint.exists():
        with np.load(checkpoint, allow_pickle=False) as saved:
            ...
            truncated, seconds, nll_sum = int(saved['truncated']), float(saved['seconds']), float(saved['nll_sum'])
            if updates:
                optimizer.state[beta] = dict(step=torch.tensor(float(updates)),
    ...
    def save():
        ...
        atomic_npz(checkpoint, identity=np.asarray(identity), beta=beta.detach().cpu().numpy(),
            cursor=np.asarray(cursor), updates=np.asarray(updates), truncated=np.asarray(truncated),
            seconds=np.asarray(seconds + time.monotonic() - started), nll_sum=np.asarray(nll_sum),
    ...
    while cursor < len(schedule):
        ...
        for index in batch:
            loss, metrics = coefficient_loss(target, rows[index], beta, dictionary, layer,
                                             condition['positions'], config)
            (loss / len(batch)).backward()
            truncated += int(metrics['truncated'])
            nll_sum += metrics['nll']
        if beta.grad is None or not torch.isfinite(beta.grad).all():
            raise ValueError('Missing/nonfinite beta gradient; GPU backward preflight failed')
        ...
        if updates % c['save_every_steps'] == 0 or cursor == len(schedule):
            save()
    return dict(coefficients=beta.detach().cpu().tolist(), max_update_norm=c['max_update_norm'],
        training_examples=cursor, optimizer_steps=updates, training_reference_truncated=truncated,
...
def paired_power_plan(differences, target_effect=.05, power=.8, alpha=.05):
    ...
    n = math.ceil((norm.ppf(1 - alpha / 2) + norm.ppf(power))**2 *
                  float(np.var(values, ddof=1)) / target_effect**2)
    return dict(required_groups=max(2, n), status='approximate_planning_only',
                target_absolute_effect=target_effect, alpha=alpha, power=power)
