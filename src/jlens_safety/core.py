...
def linear_probe(x, y, seed):
    ...
    clf = LogisticRegression(C=0.1, max_iter=1000, random_state=seed).fit(scaler.transform(x), y)
...
class Intervention:
    ...
    def __call__(self, module, inputs, output):
        ...
        if not first and h.shape[1] != 1:
            raise ValueError('Decode requires use_cache=True; refusing repeated full-prefix steering')
        if first:
            if self.gate is None:
                self.gate_on = True
            else:
                vector, mean, std, threshold = self.gate
                score = (h[0, -1].float() @ torch.as_tensor(vector, device=h.device) - mean) / std
                self.gate_on = bool(score.item() >= threshold)
        active = (self.mode != 'prefill' or first) and (self.mode != 'gated' or self.gate_on)
...
def intervention_from_condition(condition, artifacts):
    ...
    if method == 'random':
        direction = unit(np.random.default_rng(condition['random_seed']).normal(size=len(a['j_direction'])))
    elif method == 'combined':
        # Norm-matched mixture of independently selected signed baseline and J direction.
        mixture = a['j_direction'] * condition['j_alpha'] + a['residual_direction'] * condition['residual_alpha']
        if np.linalg.norm(mixture) < 1e-10:
            # Exact cancellation is a genuine no-op, not an arbitrary replacement direction.
            return Intervention(a['j_direction'], a['residual_scale'], 0.0, condition['mode'])
        direction = unit(mixture)
    else:
        direction = a[{'jlens': 'j_direction', 'residual': 'residual_direction',
