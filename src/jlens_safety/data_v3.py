...
TYPES = ('vanilla_harmful', 'vanilla_benign', 'adversarial_harmful', 'adversarial_benign')
...
def wild_rows(records, evaluation=False, invalid_training_rows=None):
    ...
    for i, r in enumerate(records):
        ...
        prompt = r.get('adversarial' if kind.startswith('adversarial') else 'vanilla', '').strip()
        completion = r.get('completion', '').strip()
        if not evaluation and invalid_training_rows is not None and (not prompt or not completion):
            invalid_training_rows.append(dict(id=f'wj-train-{i}', category=kind,
                reason='missing_prompt' if not prompt else 'missing_reference_completion'))
            continue
        if not prompt:
        ...
        result.append(dict(id=f'wj-{"eval" if evaluation else "train"}-{i}', prompt=prompt,
            base_prompt=base, group='request-' + digest(normalized(base))[:24],
            harmful=int(kind.endswith('_harmful')), category=kind, corpus='wildjailbreak',
            answer=None, completion=completion, split='test' if evaluation else None,
            group_source='vanilla' if r.get('vanilla', '').strip() else 'prompt_only'))
    return result
...
def select_training(rows, config):
    ...
    for (split, kind), values in sorted(buckets.items()):
        ...
        if len(values) < count:
            raise ValueError(f'Only {len(values)} eligible {split}/{kind}; require {count}')
...
def add_harmbench_attacks(behaviors, config):
    ...
    if mode == 'published_human_jailbreaks':
        ...
        for r in behaviors:
            ...
            cases[r['behavior_id']] = [text + '\n\n' + r['behavior'] for text in templates[:count]]
        attack_hash = digest(dict(source_sha256=hashlib.sha256(raw).hexdigest(), cases=cases))
    elif mode != 'file':
        raise ValueError('Unknown harmbench_attack_mode')
    elif not path:
        if config['data'].get('require_harmbench_attacks', True):
            raise ValueError('Supply harmbench_attack_file and harmbench_attack_source before prepare; '
    ...
    for r in behaviors:
        ...
        if not isinstance(values, list) or not values or not all(isinstance(v, str) and v.strip() for v in values):
            raise ValueError(f'Missing/invalid adversarial test cases for {r["behavior_id"]}')
        for i, text in enumerate(values):
            result.append({**r, 'id': r['id'] + f'-attack-{i}', 'prompt': text,
...
def prepare(config, run):
    ...
    if d.get('harmbench_attack_mode', 'file') == 'file' and d.get('require_harmbench_attacks', True) and not d.get('harmbench_attack_file'):
        raise ValueError('Set data.harmbench_attack_file and source; see docs/JLENS_SIGNED_TOKENS.md')
    wj, invalid_training_rows = {}, []
    for split in ('train', 'eval'):
        path = hf_hub_download(d['wildjailbreak_repo'], f'{split}/{split}.tsv',
                              repo_type='dataset', revision=d['wildjailbreak_revision'])
        hashes['wildjailbreak_' + split] = file_hash(path)
        with open(path, newline='', encoding='utf-8') as handle:
            wj[split] = wild_rows(csv.DictReader(handle, delimiter='\t'), split == 'eval',
                                 invalid_training_rows if split == 'train' else None)
    atomic_json(run/'invalid_training_rows.json', invalid_training_rows)
    print(f'WildJailbreak loaded; excluded {len(invalid_training_rows)} malformed training rows; test unchanged', flush=True)
    xs, hashes['xstest'] = fetch_csv(f'https://raw.githubusercontent.com/paul-rottger/xstest/{d["xstest_revision"]}/xstest_prompts.csv')
    hb, hashes['harmbench'] = fetch_csv(f'https://raw.githubusercontent.com/centerforaisafety/HarmBench/{d["harmbench_revision"]}/data/behavior_datasets/harmbench_behaviors_text_test.csv')
    test = list(wj['eval'])
    for r in xs:
        if r['label'] not in ('safe', 'unsafe'):
            raise ValueError('Unexpected XSTest label')
        test.append(dict(id='xs-' + r['id'], prompt=r['prompt'], base_prompt=r['prompt'],
            group='xs-focus-' + normalized(r['focus']), harmful=int(r['label'] == 'unsafe'),
            split='test', category=r['type'], corpus='xstest', answer=None))
    behaviors = [dict(id='hb-' + r['BehaviorID'], behavior_id=r['BehaviorID'],
        behavior=r['Behavior'], prompt=r['Behavior'], base_prompt=r['Behavior'],
        group='hb-' + r['BehaviorID'], harmful=1, split='test', category=r['SemanticCategory'],
        corpus='harmbench_direct', answer=None) for r in hb if r['FunctionalCategory'] == 'standard']
    ...
    too_long = [r['id'] for r in test if r['prompt_token_count'] > config['model']['max_prompt_tokens']]
    if too_long:
    ...
    for split, original in [('validation', 'train'), ('test', 'test')]:
        path = hf_hub_download('openai/gsm8k', f'main/{original}-00000-of-00001.parquet',
                              repo_type='dataset', revision=d['gsm8k_revision'])
        ...
        for i in indices[:count]:
            ...
            rows.append(dict(id=f'gsm-{original}-{i}', group=f'gsm-{original}-{i}',
                ...
                harmful=0, split=split, category='math', corpus='gsm8k',
                answer=r['answer'].split('####')[-1].strip().replace(',', '')))
    validate_rows(rows)
    atomic_json(run / 'data_manifest.json', dict(rows_hash=digest(rows), sha256=hashes,
...
def subset(rows, split, per_type, gsm_count, seed):
    ...
    for i, kind in enumerate(TYPES):
        pool = [r for r in rows if r['split'] == split and r['corpus'] == 'wildjailbreak' and r['category'] == kind]
